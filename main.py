...
from lipreading.utils import plot_confusion_matrix
...
def calc_confusion_matrix(model, dset_loader, criterion):
    model.eval()
    predictions = []
    corrects = []
    with torch.no_grad():
        for batch_idx, (input, lengths, labels) in enumerate(tqdm(dset_loader)):
            ...
            corrects += labels.tolist()

        confusionM = confusion_matrix(corrects, predictions)
        with open('confusion_matrix.txt', 'w') as f:
            f.write(str(confusionM.tolist()))
        plot_confusion_matrix(confusionM, list(range(100)), "confusion_matrix.png")
